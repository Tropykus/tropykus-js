...
function validateAsset(
	...
	if (typeof asset !== "string" || asset.length < 1) {
		throw Error(
			errorPrefix + "Argument `" + argument + "` must be a non-empty string."
		);
	}
	...
	const underlyingAddress =
		address[getNetNameWithChainId(this._network.id)][underlyingName];
...
export async function getPrice(
	...
	const priceFeedAddress =
		address[getNetNameWithChainId(this._network.id)].PriceFeed;
	const trxOptions: CallOptions = {
		_compoundProvider: this._provider,
		abi: abi.PriceFeed,
	};

	const assetUnderlyingPrice = await eth.read(
		priceFeedAddress,
		"price",
		[underlyingName],
		trxOptions
	);
	const inAssetUnderlyingPrice = await eth.read(
		priceFeedAddress,
		"price",
		[inAssetUnderlyingName],
		trxOptions
	...
	if (assetIsCToken) {
		assetCTokensInUnderlying = await cTokenExchangeRate.bind(this)(
			cTokenAddress,
			cTokenName,
			underlyingDecimals
		);
